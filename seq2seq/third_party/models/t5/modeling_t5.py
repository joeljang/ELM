...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, adapter_config=None, prefix_emb=None, attn_prefix_tuning=False, mul_prefix_emb=None, model_dim=768, attn_method="linear", shared_attn=False, ignore_target=False, temperature=2000):
        ...
        if inputs_embeds is None:
            ...
            if self.append_attn_prefix:
                ...
                if self.ignore_target is False:
                    ...
                    mul_prefix_emb_added = torch.cat((self.mul_prefix_emb.repeat(inputs_embeds.shape[0], 1, 1, 1), target_prompts.unsqueeze(1)), dim=1)
                    avg_mul_prefix_emb, _ = torch.max( mul_prefix_emb_added, 2)
                else:
                    mul_prefix_emb_added = self.mul_prefix_emb.repeat(inputs_embeds.shape[0], 1, 1, 1)
                    avg_mul_prefix_emb, _ = torch.max( mul_prefix_emb_added, 2)
                ...
                if self.append_attn_prefix:
                    ...
                    # 3. Down and up projection-based implementation.
                        ...
                        attn_scores = (torch.ones(mul_prefix_emb_added.size(0), mul_prefix_emb_added.size(1)) / mul_prefix_emb_added.size(1)).cuda()
                    else:
                    ...
                    if self.ignore_target is False:
                        if self.shared_attn is True and task_ids is not None:
                            ...
                            soft_prompts = soft_prompts + self.prefix_emb[0].unsqueeze(0).repeat(inputs_embeds.shape[0], 1, 1)
                        else:
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config, adapter_config=None):
        ...
        if self.shared_attn is True:
            self.prefix_shared = nn.Parameter(torch.zeros((config.num_target, self.prefix_dim, config.d_model))) if self.prefix_tuning else None 
        else:
        ...
        self.mul_prefix_emb = nn.Parameter(torch.zeros((self.prefix_num, self.prefix_dim, config.d_model))) if self.prefix_tuning and self.attn_prefix_tuning else None
        #############################################################
        encoder_config = copy.deepcopy(config)
        encoder_config.is_decoder = False
        encoder_config.use_cache = False
        encoder_config.is_encoder_decoder = False
        self.encoder = T5Stack(encoder_config, self.shared, adapter_config=adapter_config, prefix_emb=self.prefix_shared, attn_prefix_tuning=self.attn_prefix_tuning, mul_prefix_emb=self.mul_prefix_emb, model_dim=config.d_model, attn_method=self.attn_method, shared_attn=self.shared_attn, ignore_target=self.ignore_target, temperature=self.temperature)
        ...
        self.decoder = T5Stack(decoder_config, self.shared, adapter_config=adapter_config, prefix_emb=self.prefix_shared, attn_prefix_tuning=self.attn_prefix_tuning, mul_prefix_emb=self.mul_prefix_emb, model_dim=config.d_model, attn_method=self.attn_method, shared_attn=self.shared_attn, ignore_target=self.ignore_target, temperature=self.temperature)
    ...
    def update_prefix_weights(self, prefix_embeddings, target_embedding=None):
        ...
        if len(prefix_embeddings) == 1:
            ...
            if target_embedding is not None:
                ...
                sum_sims = torch.sum(torch.Tensor([prefix_emb_similarity(emb, target_embedding) for emb in prefix_embeddings ]))
                W_weighting = torch.Tensor([prefix_emb_similarity(emb, target_embedding) / sum_sims for emb in prefix_embeddings]).detach()
                res = torch.einsum('mld,m->ld', torch.stack([emb.cuda() for emb in prefix_embeddings]), W_weighting.cuda())
                self.prefix_shared.data = res
            else:
                self.W_weighting = nn.Parameter(torch.rand(len(prefix_embeddings)))
                res = torch.einsum('mld,m->ld', torch.stack([emb.cuda() for emb in prefix_embeddings]), self.W_weighting.cuda())
                self.prefix_shared.data = res
