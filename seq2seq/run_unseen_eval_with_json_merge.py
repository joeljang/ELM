...
def main():
    ...
    #data_args.txt_save_dir = "output_logs_unseen_eval_prompt_retriever_full_512"
    data_args.txt_save_dir = "output_logs_unseen_eval_prompt_oracle_full_512"
    data_args.expert_mapping_dir="./merge/oracle_sorted.json"
    #data_args.expert_mapping_dir="./merge/ST_choice_instance_top1_32.json"
    data_args.top_k_merge = 1
    data_args.per_device_eval_batch_size = 32

    # Loading Eval -> Expert Mapping
    expert_mapping = json.load(open(data_args.expert_mapping_dir,"r"))
    TOPK_MERGE = data_args.top_k_merge
    ...
    if training_args.do_eval:
        max_target_lengths = [AutoTask.get(dataset_name, dataset_config_name, prompt=eval_prompt).get_max_target_length(
            tokenizer=tokenizer, default_max_length=data_args.max_target_length)
            for dataset_name, dataset_config_name, eval_prompt in zip(data_args.eval_dataset_name, data_args.eval_dataset_config_name, data_args.eval_prompts)]
        with open('/home/joel_jang/seungone/RoE/seq2seq/data/manual/unseen_eval_full.json','r') as f:
            eval_datasets = {}
            ...
            for eval_dataset in data:
                ...
                for eval_prompt in data[eval_dataset]:
                    tmp_dict = {"source":[],"target":[],"labels_list":[],'extra_fields':[],'task':[]}
    ...
    eval_name_mapping = {
        ...
        "anli_r2": "anli_dev_r2",
        "anli_r3": "anli_dev_r3",
        "super_glue_copa":"super_glue/copa",
        "super_glue_cb": "super_glue/cb",
        "super_glue_rte": "super_glue/rte",
        "super_glue_wsc.fixed": "super_glue/wsc.fixed",
        "super_glue_wic": "super_glue/wic",
    ...
    expert_name_mapping = {
        ...
        "glue/mrpc": "mrpc",
        "glue_mrpc": "mrpc",
        "adversarial_qa/adversarialQA": "adversarial_qa",
        "duorc/ParaphraseRC": "duorc",
        "hotpot_qa/fullwiki": "hotpot_qa",
        "cnn_dailymail/3.0.0": "cnn_dailymail"
        }
    merge_expert_mapping = {
        "cos_e/v1.11": "cos_e",
        "wiki_hop/original": "wiki_hop",
        "paws/labeled_final": "paws",
        "glue/qqp": "glue_qqp",
    ...
    if training_args.do_eval:
        ...
        if model_args.shared_attn is True:
            ...
            # Loading eval -> expert combo mapping file!
            eval_cnt = 0
            adapter_param_names = []
            for task, eval_dataset in eval_datasets.items():
                print('$$$$$$$$$$$$@@@@@@@@@@@@@@')
                ...
                print('$$$$$$$$$$$$@@@@@@@@@@@@@@')
                task_s = task.split('*')
                e_dataset, e_prompt = task_s[0], task_s[1]
                if 'oracle' in data_args.expert_mapping_dir:
                    e_dataset = eval_name_mapping[e_dataset]
                task_expert = f"{e_dataset}@{e_prompt}"
                #task_expert = task.replace('*', '@')

                expert_list = expert_mapping[task_expert]
                merge_list = []
                if len(expert_list) < TOPK_MERGE:
                    TOPK_MERGE = len(expert_list)
                for i in range(TOPK_MERGE):
                    merge_list.append(expert_list[i])
                
                # Loading the merged experts
                merge_params = []
                merge_expert_name = []
                for i in range(len(merge_list)):
                    selected_expert = merge_list[i]
                    adapter_params = {}
                    #lst = os.listdir(model_args.adapter_dir)
                    merge_expert_name.append(selected_expert)
                    selected_expert_s = selected_expert.split('@')
                    selected_dataset, selected_prompt = selected_expert_s[0], selected_expert_s[1]
                    if selected_dataset in expert_name_mapping:
                        selected_dataset = expert_name_mapping[selected_dataset]
                    selected_expert = selected_expert.replace('@', '/')
                    expert_dir = f"expert_weights/{selected_dataset}/{selected_prompt}"
                    lst = os.listdir(os.path.join(expert_dir,"adapter_params"))
                    for path in lst:
                        #full_path = model_args.adapter_dir + '/' + path
                        full_path = os.path.join(expert_dir,"adapter_params",path)
                        params = torch.load(full_path)
                        path_ = path.split('.')
                        path = ".".join(path_[:-1])
                        adapter_params[path] = params
                    merge_params.append(adapter_params)
                # Saving adpater parameter name when loading for the first time
                if eval_cnt==0:
                    for name, param in trainer.model.named_parameters():
                        if param.requires_grad:
                ...
                for name, param in trainer.model.named_parameters():
                    ...
                    if name in adapter_param_names:
                        load_cnt+=1
                        params_sum = None
                        for i in range(len(merge_params)):
                            print(i)
                            expert_name = merge_expert_name[i]
                            expert_name = expert_name.split('@')[0]
                            print(name)
                            if expert_name in merge_expert_mapping:
                                expert_name = merge_expert_mapping[expert_name]
                            new_name = name.replace(adapter_config.adapters_cur_training_task ,expert_name)
                            print(new_name)
                            if i==0:
                                params_sum = merge_params[i][new_name]
                            else:
                                params_sum = torch.add(params_sum, merge_params[i][new_name])
                        merged_params = torch.div(params_sum, len(merge_params))
                        param.data = merged_params.cuda()
                        #param.data = adapter_params[name].cuda()
                print(f'load count: {load_cnt}')
                print(f'Finished loading {len(adapter_params)} number of adapter parameter files')
