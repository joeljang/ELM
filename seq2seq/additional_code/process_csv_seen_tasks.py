...
for request_task in tqdm(task_list):
    for request_prompt in task_list[request_task]['prompts']:
        ...
        for filename in os.listdir(f'./output_logs_seen_eval/{request_task}'):
        #for filename in os.listdir(f'./output_logs_task_full_lm2'):
            ...
            try:
                ...
                if(train_task == target_task) and (train_prompt == target_prompt):
                    with open(f'./output_logs_seen_eval/{request_task}/{request_task}*{train_prompt}-{target_task}*{target_prompt}.txt','r') as f:
                        for row in f:
                            if row[0:2] != ">>":
                                continue
                            else:
...
x.to_csv(f'./csv_results_seen_eval/seen_result_300.csv',index=False)