#CUDA_VISIBLE_DEVICES="" python3 run_eval.py ../../configs/experiment/[data_name]/[dataset_name][#num].json

json_file_DIR = "/home/joel_jang/seungone/RoE/configs/experiment"
bash_file_DIR = "/home/joel_jang/seungone/RoE/seq2seq/bash_files"
...
bash_file_name_list = []
for data_name in os.listdir(json_file_DIR):
    if data_name != "t0.json":
        for config_name in os.listdir(os.path.join(json_file_DIR,data_name)):
            if 'all' not in config_name:
                config_dir = os.path.join("../../configs/experiment",data_name,config_name)
                bash_file_name_list.append(config_dir)
print(len(bash_file_name_list))
#create bash files
cur_bash_idx =0
cur_bash_num=0
max_num_per_gpu = 24
for bash_file in bash_file_name_list:
    with open(os.path.join(bash_file_DIR,f"{cur_bash_idx}.sh"),"a") as f:
        f.write(f"CUDA_VISIBLE_DEVICES='{cur_bash_idx}' python3 ../run_instance_eval.py {bash_file}\n")
    cur_bash_num+=1
    if cur_bash_num >max_num_per_gpu:
        cur_bash_idx+=1
        cur_bash_num=0