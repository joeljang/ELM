...
for filename in os.listdir(f'./output_logs_unseen_eval_ct0_replication/{request_task}'):
    try:
        ...
        with open(f'./output_logs_unseen_eval_ct0_replication/{request_task}/{train_task}*{train_prompt}-{target_task}*{target_prompt}.txt','r') as f:
            for row in f:
                if row[0:2] != ">>":
                    continue
                else:
        ...
        continue
result['overall'] = overall_accuracy/113
...
x.to_csv(f'./csv_results_unseen_eval_full/output_logs_ct0.csv',index=False)