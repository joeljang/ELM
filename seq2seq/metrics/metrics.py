...
def bertscore(predictions, targets) -> dict:
    scorer = load_metric('bertscore',model_type="microsoft/deberta-large-mnli")
    scorer.add_batch(predictions=predictions, references=targets)
    score = scorer.compute(model_type="microsoft/deberta-large-mnli")
    print('^^^^^^^^^^^^^^^^^^^^^^^^^^^^^')
    print(score)
    print('^^^^^^^^^^^^^^^^^^^^^^^^^^^^^')
    return{"bertscore":sum(score['precision'])/len(score['precision'])}
