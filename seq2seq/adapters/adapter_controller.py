...
class AdapterController(nn.Module):
    ...
    def get_adapter(self, task):
        ...
        # if task=="superglue-rte":
        #     return self.adapters['paws']
        # elif task=="triviaqa":
        #     return self.adapters['hotpotqa']
        # elif task=="lama":
        #     return self.adapters['hotpotqa']
        
        # TODO : Fix during only evaluation
    ...
    def forward(self, inputs, task):
        ...
        adapter = self.get_adapter(task)
        z = self.pre_layer_norm(
            inputs) if self.add_layer_norm_before_adapter else inputs
        outputs = adapter(z)
        if self.add_layer_norm_after_adapter:
