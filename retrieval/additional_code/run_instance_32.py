...
for key in eval_dict:
    ...
    for row in samples:
        ...
        if id==32:
            ...
            task_level[f'{original_dataset_name}@{prompt_name}'] = picked_expert
            stats = {}
            id+=1
            continue
        elif id>32:
...
for key in eval_dict:
    ...
    for row in samples:
        ...
        if f'{original_dataset_name}@{prompt_name}' not in seen_prompt:
            if id!=0:
                eval_prompt_score = accum_score / id
                entries.append(eval_prompt_score)
            accum_score = 0
            id = 0
            seen_prompt[f'{original_dataset_name}@{prompt_name}'] = 0
        picked_expert = task_level[f'{original_dataset_name}@{prompt_name}']
        key = f'{picked_expert}@{original_dataset_name}@{prompt_name}@{id}'
        print(id, key)
        accum_score += score[key]
        id+=1
    eval_prompt_score = accum_score / id
    entries.append(eval_prompt_score)

pd.DataFrame([entries]).to_csv(f'results/{config.method}_{config.text_format}_top{config.top_k}_32.csv', index=False) 
print(f'good job')