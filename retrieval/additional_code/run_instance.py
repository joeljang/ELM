...
for key in eval_dict:
    ...
    for row in samples:
        ...
        if f'{original_dataset_name}@{prompt_name}' not in seen_prompt:
            ...
            #print(f'stats: {stats}')
            #print(f'{original_dataset_name}@{prompt_name}')
            stats = {}
        text = row["source"]
        target = row["target"]
        if 'choice' in config.text_format:
        ...
        if f'{expert_dataset}@{expert_prompt}' not in stats:
            ...
            stats[f'{expert_dataset}@{expert_prompt}']+=1
    eval_score = accum_score / len(samples)
    print(f'Eval dataset: {dataset_name}. Score: {eval_score}')
    entries.append(eval_score)

pd.DataFrame([entries]).to_csv(f'results/{config.method}_{config.text_format}_top{config.top_k}.csv', index=False) 
print(f'good job')