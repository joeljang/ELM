...
train_dict = json.load(open("retrieval_data/train_examples_100.json","r"))
eval_dict = json.load(open("retrieval_data/target_eval_300.json","r"))
#eval_dict = json.load(open("retrieval_data/generative_eval_300.csv","r"))

expert_name_to_scores = {}
#df = pd.read_csv('retrieval_data/dataset_experts.csv')
df = pd.read_csv('retrieval_data/generative_eval_300.csv')
...
for index,row in df.iterrows():
    if index==0:
        ...
        expert_name_to_scores[train_dataset] = row[2:]

if 'ST' in config.method or 'T0' in config.method:
    all_embeddings = np.load(f"retrieval_data/indexes/train_100_{config.method}_{config.text_format}.npy")
...
for i in range(len(eval_datasets)):
    ...
    for name in evals:
        if name in dataset:
            dataset = name
    picked_expert = task_level[f'{dataset}@{prompt}'].split('@')[0]
    score = expert_name_to_scores[picked_expert][i]
    entries.append(score)

print(entries)
pd.DataFrame([entries]).to_csv(f'results/dataset_level_{config.method}_{config.text_format}_32.csv', index=False) 
print(f'good job')